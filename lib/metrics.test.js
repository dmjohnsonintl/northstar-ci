...
// --- fixPrOutcomes: the two live bugs it exists to prevent (2026-08-05) ---

const CANARY_RUNS = [
  { databaseId: 30897069091, workflowName: 'Northstar canary' },
  { databaseId: 30806392264, workflowName: 'Northstar canary' },
  { databaseId: 555, workflowName: 'Northstar pipeline' },
];

test('fixPrOutcomes: a merged PR is never also counted as closed-unmerged', () => {
  // `gh pr list --state closed` returns merged PRs too, so the gather hands us the
  // same PR in both lists. Verified live: council-principis closed=50, merged=48,
  // overlap=48. Counting the merge twice caps acceptance at 50%.
  const prs = [
    { headRefName: 'ns/fix/555', state: 'MERGED', mergedAt: '2026-08-01T00:00:00Z' },
    { headRefName: 'ns/fix/556', state: 'CLOSED', mergedAt: '2026-08-01T00:00:00Z' },
  ];
  const o = fixPrOutcomes(prs, CANARY_RUNS);
  assert.equal(o.merged, 2);
  assert.equal(o.closedUnmerged, 0, 'a PR with mergedAt is merged, whatever its state says');
  assert.equal(agentHealth({ fixPrsMerged: o.merged, fixPrsClosedUnmerged: o.closedUnmerged }).humanAcceptanceRate, 100);
});

test('fixPrOutcomes: canary self-cleanup is not a human rejection', () => {
  // The canary opens ns/fix/<run_id> and its own cleanup closes it. Unlabeled, so
  // only the run id distinguishes it from a real fix PR.
  const prs = [
    { headRefName: 'ns/fix/30897069091', state: 'CLOSED', mergedAt: null }, // canary
    { headRefName: 'ns/fix/30806392264', state: 'CLOSED', mergedAt: null }, // canary
    { headRefName: 'ns/fix/555', state: 'MERGED', mergedAt: '2026-08-01T00:00:00Z' },
  ];
  const o = fixPrOutcomes(prs, CANARY_RUNS);
  assert.equal(o.canaryExcluded, 2);
  assert.equal(o.merged, 1);
  assert.equal(o.closedUnmerged, 0, 'canary cleanup must not read as a rejection');
});

test('fixPrOutcomes: an OPEN PR is undecided — neither merged nor rejected', () => {
  const prs = [{ headRefName: 'ns/fix/555', state: 'OPEN', mergedAt: null }];
  const o = fixPrOutcomes(prs, CANARY_RUNS);
  assert.deepEqual({ m: o.merged, c: o.closedUnmerged }, { m: 0, c: 0 });
});

test('fixPrOutcomes: a genuine human rejection still counts', () => {
  const prs = [
    { headRefName: 'ns/fix/555', state: 'CLOSED', mergedAt: null }, // real run, closed by a person
    { headRefName: 'ns/fix/556', state: 'MERGED', mergedAt: '2026-08-01T00:00:00Z' },
  ];
  const o = fixPrOutcomes(prs, CANARY_RUNS);
  assert.equal(o.closedUnmerged, 1);
  assert.equal(o.merged, 1);
  assert.equal(agentHealth({ fixPrsMerged: 1, fixPrsClosedUnmerged: 1 }).humanAcceptanceRate, 50);
});

test('fixPrOutcomes: non-ns/ PRs are ignored entirely', () => {
  const prs = [{ headRefName: 'feat/whatever', state: 'CLOSED', mergedAt: null }];
  const o = fixPrOutcomes(prs, CANARY_RUNS);
  assert.deepEqual({ m: o.merged, c: o.closedUnmerged }, { m: 0, c: 0 });
});

test('fixPrOutcomes: without runs it returns null, never a canary-polluted count', () => {
  const prs = [{ headRefName: 'ns/fix/30897069091', state: 'CLOSED', mergedAt: null }];
  for (const runs of [undefined, null, 'nope']) {
    const o = fixPrOutcomes(prs, runs);
    assert.equal(o.merged, null);
    assert.equal(o.closedUnmerged, null);
  }
  // …and that null must propagate to an omitted rate, not a fabricated 0%.
  assert.equal(agentHealth({ fixPrsMerged: null, fixPrsClosedUnmerged: null }).humanAcceptanceRate, null);
});

test('fixPrOutcomes: canary workflow name is configurable', () => {
  const runs = [{ databaseId: 77, workflowName: 'Nightly canary (custom)' }];
  const prs = [{ headRefName: 'ns/fix/77', state: 'CLOSED', mergedAt: null }];
  assert.equal(fixPrOutcomes(prs, runs, { canaryWorkflow: 'Nightly canary (custom)' }).canaryExcluded, 1);
  assert.equal(fixPrOutcomes(prs, runs).canaryExcluded, 0, 'default name must not match a custom canary');
});
