...
test('renderDashboard contains the key numbers and no fabricated cost zeros', () => {
  ...
  assert.match(md, /## Cost/); // honest about cost — no records on this model, so falls back, never fabricated
});
...
test('dashboard renders coordination health + regression growth from traces', () => {
  ...
  // Cost section renders (falls back to — since this model has no `cost` records).
