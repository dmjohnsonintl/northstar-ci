...
// Human-acceptance counts come from the raw PR list, not from pre-counted numbers.
// The gather cannot compute them correctly on its own: `gh pr list --state closed`
// includes merged PRs, and the canary's cleanup closes its own ns/fix PRs. Both are
// filtered here. Absent --prs leaves the pre-existing fields untouched.
const prsForAcceptance = readJson(arg('prs'), null);
if (prsForAcceptance) {
  const outcomes = fixPrOutcomes(prsForAcceptance, runs, { canaryWorkflow });
  agentEvents.fixPrsMerged = outcomes.merged;
  agentEvents.fixPrsClosedUnmerged = outcomes.closedUnmerged;
}
