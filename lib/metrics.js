...
// Split Northstar fix PRs into the two outcomes a HUMAN decided, excluding the
// canary's own self-cleanup. PURE.
//
// Two traps this exists to avoid, both found live on 2026-08-05:
//
//  1. `gh pr list --state closed` INCLUDES merged PRs (GitHub models a merge as a
//     close). Counting that as "closed unmerged" double-counts every merge into the
//     denominator, capping the acceptance rate at 50% even if every PR were merged.
//  2. The nightly canary opens a real `ns/fix/<run_id>` PR and its own cleanup step
//     closes it. Those are unlabeled and indistinguishable from a genuine fix PR by
//     name alone, so every canary run silently registered as a human rejection.
//
// Canary runs are identified from `runs` (already gathered for pipeline health) by
// matching the run id embedded in `ns/fix/<run_id>` against runs whose workflowName
// is the canary's. No new gather, no new label, no new permission.
//
// OPEN PRs count toward neither outcome — undecided is not rejected.
//
// `runs` is REQUIRED to filter. Without it the canary cannot be excluded, and a
// canary-polluted rate is worse than no rate, so every field returns null and the
// dependent rule is omitted rather than fed a fabricated number.
function fixPrOutcomes(prs, runs, opts = {}) {
  const canaryWorkflow = opts.canaryWorkflow || 'Northstar canary';
  if (!Array.isArray(prs) || !Array.isArray(runs)) {
    return { merged: null, closedUnmerged: null, canaryExcluded: null };
  }
  const canaryRunIds = new Set(
    runs
      .filter((r) => r && r.workflowName === canaryWorkflow && r.databaseId != null)
      .map((r) => String(r.databaseId)),
  );
  const isCanaryPr = (headRefName) => {
    const m = /^ns\/fix\/(\d+)$/.exec(String(headRefName || ''));
    return m ? canaryRunIds.has(m[1]) : false;
  };

  let merged = 0;
  let closedUnmerged = 0;
  let canaryExcluded = 0;
  for (const pr of prs) {
    if (!pr || typeof pr.headRefName !== 'string') continue;
    if (!pr.headRefName.startsWith('ns/')) continue;
    if (isCanaryPr(pr.headRefName)) {
      canaryExcluded += 1;
      continue;
    }
    if (pr.mergedAt != null) merged += 1;
    else if (String(pr.state || '').toUpperCase() === 'CLOSED') closedUnmerged += 1;
  }
  return { merged, closedUnmerged, canaryExcluded };
}
