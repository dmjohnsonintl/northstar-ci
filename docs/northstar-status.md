# Northstar status — dmjohnsonintl/northstar-ci

_Window: last 14 days · generated 2026-07-22T15:17:54Z_

## Pipeline health

- **Runs:** 98 (95 completed)
- **Green rate:** 81.1% ████████░░ (77 passed / 18 failed)
- **Median run time:** 17s
- **CI time (window):** 31 min

| Workflow | Runs | Passed | Failed |
|---|--:|--:|--:|
| .github/workflows/min.yml | 3 | 0 | 3 |
| Northstar GC | 42 | 42 | 0 |
| aidemo | 4 | 0 | 4 |
| bugintakedemo | 1 | 1 | 0 |
| ci | 14 | 13 | 0 |
| e2edemo | 1 | 1 | 0 |
| fixdemo | 7 | 0 | 7 |
| flakydemo | 1 | 1 | 0 |
| jestdemo | 1 | 1 | 0 |
| metricsdemo | 8 | 7 | 0 |
| plain | 2 | 2 | 0 |
| pydemo | 1 | 1 | 0 |
| secretscandemo | 1 | 1 | 0 |
| selftest | 8 | 4 | 3 |
| signaldemo | 2 | 1 | 1 |
| substratedemo | 1 | 1 | 0 |
| vitedemo | 1 | 1 | 0 |

## Coverage trend

- **Latest:** 75% (Δ vs previous ±0, vs window start ±0)
- **Recent:** 75%

## Agent / model health

- **Fix success rate:** 100% (6 fixed → PR, 0 escalated to `ns:needs-human`)
- **Human acceptance:** 0%
- **Bugs reproduced:** 1 (`ns:needs-info`: 0)
- **Tests promoted to regression:** —

## Coordination health

- **Active zone claims:** 0
- **Flake signals:** 0 · **coverage-gap:** 0 · **reclaimed (crashed agents):** 0

## Regression growth

- **Promotions (window):** 5 — blessed tests added to the permanent suite

## Cost

- **Total (window):** — across 0/1 fix runs with cost data
- **Per fix run:** —
- **Tokens:** in — · out — · cache-read —
- **By layer:** unit — (1 runs)

